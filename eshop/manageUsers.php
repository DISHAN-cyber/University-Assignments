...
<body style="background-color: #74EBD5;background-image: linear-gradient(90deg,#74EBD5 0%,#9FACE6 100%);">
    ...
    <div class="container-fluid">
                ...
                <div class="col-12 mt-3 mb-3">
                    <div class="row">
                        ...
                        <div class="col-2 d-none d-lg-block bg-light py-2" onclick="viewMsgModal('<?php echo $selected_data['email']; ?>);">
                            ...
                            $img_rs = Database::search("SELECT * FROM `profile_img` WHERE
                        `user_email` = '" . $selected_data["email"] . "'");

                            if ($img_rs->num_rows > 0) {
                                $img_data = $img_rs->fetch_assoc();
                            ?>
                                <img src="<?php echo $img_data["img_path"]; ?>" style="height: 40px;margin-left: 80px;" />
                            <?php
                            } else {
                            ?>
                                <img src="resources/new_user.svg" style="height: 40px;margin-left: 80px;" />
                            <?php
                            }
                        ...
                        <div class="col-2 d-none d-lg-block bg-light py-2">
                            <?php
                            $split_data = explode(" ", $selected_data["joined_date"]);
                            ?>
                            <span class="fs-4 "><?php echo $split_data[0]; ?></span>
                        </div>

                        <?php
                        if ($selected_data["status_status_id"] == 1) {
                        ?>
                            <button class="btn btn-danger" onclick="blockUser('<?php echo $selected_data['email'] ?>');">Block</button>
                        <?php
                        } else if ($selected_data["status_status_id"] == 2) {
                        ?>
                            <button class="btn btn-succes" onclick="blockUser('<?php echo $selected_data['email'] ?>');">Block</button>
                        <?php
                        }
                        ?>
        ...
        <!-- msg modal -->
        <div class="modal" tabindex="-1" id="msgModal<?php echo $selected_data['email']; ?>">
                    ...
                    <div class="modal-footer">
                        ...
                        <div class="col-12">
                            <div class="row">
                                ...
                                <div class="col-3 d-grid">
                                    <button type="button" class="btn btn-primary" onclick="adminChat('<?php echo $selected_data['email']; ?>')">Send</button>
                                </div>
                            </div>
                        </div>
